...
function evaluateBankSets(cards) {
    ...
    while (i < cards.length) {
        ...
        if (matchedLen > 0) {
            for (let k = 0; k < matchedLen; k++) isValid[i + k] = true;
            i += matchedLen;
        } else {
            i++;
        }
...
function processOffer(room) {
    ...
    if (offerer.isBot) {
        ...
        if (validCards.length > 0) {
            ...
            chosenCardIndex = (lichocarIndex !== -1 && Math.random() < 0.5) ? lichocarIndex : 0;
        }
...
function evaluateBank(room, accepted) {
    ...
    setTimeout(() => {
        if (accepted) {
            ...
            if (hasLichocar) {
                logRoom(room, `🔥 ODHALENÝ BLUF! V nabídce byl Lichočár.`);
                let lich = gs.bank.find(b => b.card.type === 'lichocar').card;
...
function finishLayingPhase(room) {
    ...
    cards.forEach((card, idx) => {
        if (validFlags[idx]) kept.push(card);
        else returned.push(card);
    });

    if (kept.length > 0) {
        ctx.player.sets.push({ type: 'sada', cards: kept });
        logRoom(room, `✨ ${ctx.player.name} vyložil novou sadu (${kept.length} karet)!`);
    }

    if (returned.length > 0) {
        ctx.player.hand.push(...returned);
    }
...
function prepareClientState(room, mySlotId) {
    ...
    return {
        ...
        gameState: gs ? {
            ...
            logs: gs.logs,
            validFlags: gs.phase === 'LAYING_CARDS' ? evaluateBankSets(gs.layContext.cardsFromBank) : []
        } : null
    };
}
